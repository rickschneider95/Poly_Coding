...
#delete column "Service à comp?tence national"
faits_2010 <- faits_2010 %>% 
...
n_college_2010_dp <- dpp_modif_c %>% 
                        filter(nature_uai_libe == "Coll�ge") %>% ###A ourvir???????
                        group_by(dpt) %>% 
                        arrange(dpt) %>% 
...
dim(n_college_2010_dp) #96 rows -->one row contains NA
n_college_2010_dp <- n_college_2010_dp %>% drop_na()
...
#delete he last row (service � comp�tence nationale)
